...
class PostPins extends PagelinesSection {
		...
		jQuery(document).ready(function () {
			...
			<?php if(ploption('pins_loading', $this->oset) == 'infinite'): ?>
				...
				theContainer.infinitescroll({
					...
					debug : true,
					loading: {
						finishedMsg: 'No more galleries to load.'
					}
			...
			jQuery('.fetchpins a').live('click', function(e) {
				...
				jQuery.ajax({
					...
					success: function(out) {
						...
						jQuery('.fetchpins a').removeClass('loading').text('<?php _e('Load More Galleries', 'pagelines');?>');
   ...
   function section_template() { 
		...
		foreach( $this->load_posts($number_of_pins, $page, $category) as $key => $p ){
			...
			$content = sprintf(
				'%s<p class="zmt zmb headline pin-title"><a href="%s">%s</a></p>%s', 
				$meta_top,
				get_permalink( $p->ID ), 
				$p->post_title, 
				$meta_bottom
		...
		if( !empty($next_posts) ){
			...
			$next_url = sprintf('<div class="%s fetchlink" %s><a class="btn" href="%s">%s</a></div>', $class, $display, $u, __('Load More Galleries', 'pagelines'));
